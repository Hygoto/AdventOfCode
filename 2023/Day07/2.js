...
rl.on('line', (line) => {
    ...
    for (let index = 0; index < decks[decks.length - 1][0].length; index++) {
        switch (decks[decks.length - 1][0][index]) {
            case 'J':
                decks[decks.length - 1][0][index] = 1;
                break;
    ...
    const jCount = counts.splice(0, 1)[0];
    counts.sort(compareNumbers);
    if (counts[0] + jCount == 5) deckValue += 6 * Math.pow(10, 2 * (decks[decks.length - 1][0].length - 1) + 2);
    else if (counts[0] + jCount == 4) deckValue += 5 * Math.pow(10, 2 * (decks[decks.length - 1][0].length - 1) + 2);
    else if (counts[0] + jCount == 3 && counts[1] == 2) deckValue += 4 * Math.pow(10, 2 * (decks[decks.length - 1][0].length - 1) + 2);
    else if (counts[0] + jCount == 3) deckValue += 3 * Math.pow(10, 2 * (decks[decks.length - 1][0].length - 1) + 2);
    else if (counts[0] + jCount == 2 && counts[1] == 2) deckValue += 2 * Math.pow(10, 2 * (decks[decks.length - 1][0].length - 1) + 2);
    else if (counts[0] + jCount == 2) deckValue += 1 * Math.pow(10, 2 * (decks[decks.length - 1][0].length - 1) + 2);
