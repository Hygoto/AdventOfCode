...
rl.on('line', (line) => {
    ...
    tmp = grouplengths.slice(0);
    for (let index = 0; index < 4; index++) {
        grouplengths = grouplengths.concat(tmp);
    }
    line = line.split(' ')[0];
    line = line.replace(/(\.)+/g, '.');
    tmp = line;
...
function branch(line, grouplengths, previousDamaged) {
    if (pastReturns[`${line},${grouplengths},${previousDamaged}`] != undefined) return pastReturns[`${line},${grouplengths},${previousDamaged}`];
    if (line == '' && grouplengths.length == 0) pastReturns[`${line},${grouplengths},${previousDamaged}`] = 1;
    else if (line == '' && grouplengths.length == 1 && grouplengths[0] == previousDamaged) pastReturns[`${line},${grouplengths},${previousDamaged}`] = 1;
    else if (line == '') pastReturns[`${line},${grouplengths},${previousDamaged}`] = 0;
    else if (line.charAt(0) == '#' && grouplengths.length == 0) pastReturns[`${line},${grouplengths},${previousDamaged}`] = 0;
    else if (line.charAt(0) == '#' && previousDamaged >= grouplengths[0]) pastReturns[`${line},${grouplengths},${previousDamaged}`] = 0;
    else if (line.charAt(0) == '#') {
        pastReturns[`${line},${grouplengths},${previousDamaged}`] = branch(line.slice(1), grouplengths, previousDamaged+1)
    }
    else if (line.charAt(0) == '.' && previousDamaged == 0) {
        pastReturns[`${line},${grouplengths},${previousDamaged}`] = branch(line.slice(1), grouplengths, 0);
    }
    else if (line.charAt(0) == '.' && previousDamaged == grouplengths[0]) {
        pastReturns[`${line},${grouplengths},${previousDamaged}`] = branch(line.slice(1), grouplengths.slice(1), 0);
    }
    else if (line.charAt(0) == '.') {
        pastReturns[`${line},${grouplengths},${previousDamaged}`] = 0;
    }
        ...
        pastReturns[`${line},${grouplengths},${previousDamaged}`] = tmp[0] + tmp[1];
    }
    return pastReturns[`${line},${grouplengths},${previousDamaged}`];
}
